...
function CardNew() {
    ...
    const [generate_card, setGenerateCard] = useState(null);
    const [lecture_id, setLectureId] = useState(null);
    ...
    useEffect(() => {
        if (generate_card == null) {
            setLectureId(null);
            setFileId(null);
            let stringified_card = localStorage.getItem(local_storage_keys.generate_card);
            if (stringified_card != null) {
                try {
                    let json_card = JSON.parse(stringified_card);
                    if (json_card != null) {
                        setGenerateCard(json_card);
                    }
                } catch {
                    localStorage.removeItem(local_storage_keys.generate_card);
                    setGenerateCard(null);
                }
            }
        }
        else {
            if (generate_card?.lecture != null) {
                setLectureId(generate_card?.lecture?.lecture_id);
                setText(generate_card?.lecture?.text);
                setName(generate_card?.lecture?.name);
            }
            else if (generate_card?.file != null) {
                setFileId(generate_card?.file?.file_id);
                setText(generate_card?.file?.text);
                setName(generate_card?.file?.name);
            }
        }
    }, [generate_card, setGenerateCard]);
                    ...
                    generate_card != null ?
                        <div className="generate-card-header">
                            <div className="generate-card">
                                Generirano iz{lecture_id != null ? " lekcije: " : file_id != null ? " datoteke: " : ": "}
                                <Link className="generate-card-link" to={`${prefixRoute()}/${generate_card?.lecture != null ? "lecture/" : generate_card?.file != null ? "file/" : ""}${generate_card?.lecture?.lecture_id ?? generate_card?.file?.file_id ?? ""}`}>
                                    {generate_card?.lecture?.name ?? generate_card?.file?.name ?? ""}
                                </Link>
                            </div>
                            <div className="generate-card-remove" onClick={removeGenerateCard}>Ukloni</div>
                        </div>
                        : null
                }
